...
> **TODO（Task 6で確定）:** 以下の「入力する」表・対応モデル一覧は、Task 4 のUI実装完了後に実際のフォーム項目・登録モデル一覧に合わせて更新してください。現時点では新スコープ（Chat/CLI/code review、機能別オーバーヘッド、過去会話ターン数）に合わせた仮の記述です。
...
2. `src/rates.js` を編集
   - `MODEL_RATES`: モデル別クレジット単価（4フィールド: input / output / cachedInput / cachedWrite）
