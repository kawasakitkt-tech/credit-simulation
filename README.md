...
| 参照情報の再利用状況 | 参照コード・設計書・添付ファイルなど、同じ参照情報を同一セッション内でどの程度再利用するかを選択（初回依頼〜4往復以上）。Copilot CLI 選択時のみ「同一セッション継続」の追加チェックあり |

※ system prompt・tool definitions 等の見えない文脈分は、機能ごとの概算オーバーヘッドとして自動加算されます。
※ キャッシュ入力割合は利用者が数値を直接入力するのではなく、上記の選択から自動推定されます。詳細は「キャッシュ推定について」を参照してください。
...
本ツールでは、Cached Input割合を利用者が自由入力する方式ではなく、
参照情報の再利用状況から自動推定します。
...
この値はGitHub公式の固定値ではなく、事前見積もり用の保守的な仮定です。
実際のCached Input量は、モデル・セッション継続状況・参照情報の一致度・ツール設定変更などにより変動します。
...
7. 参照情報の再利用状況: **初回依頼（0%）**
8. 「クレジット消費量を計算する」をクリックし、消費クレジット・USD・内訳・適用キャッシュ率・モデル比較表を確認する

### シナリオ2: code review で ~200行の PR をレビューする

1. 機能: **Copilot code review**
2. 変更行数（diff行数）: **200**
3. フォーム内に「変更行数 × 暫定係数で概算しています」という注意文が表示されることを確認する
4. 「クレジット消費量を計算する」をクリックし、概算クレジットを確認する。実際の消費量とは差が出る可能性がある点に注意する
